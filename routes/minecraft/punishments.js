...
import { MinecraftUserModel, MinecraftPunishmentModel } from "../../models/minecraft";
const router = express.Router();

router.post('/mc/player/issue_punishment', verifyServer, function(req, res) {
    if (req.body.name) {
        MinecraftUserModel.find({nameLower: req.body.name.toLowerCase()}).sort('-lastOnlineDate').limit(1).exec(function(err, users){
            var punished = users[0];
            if (punished) {
                MinecraftUserModel.find({uuid: req.body.punisherUuid}).sort('-lastOnlineDate').limit(1).exec(function(err, punishers){
...
router.post('/mc/player/revert_punishment', verifyServer, function(req, res) {
    MinecraftPunishmentModel.findOne({ _id: req.body.id }, function(err, punishment) {
        var success = punishment.reverted == false;
        if (err) console.log(err);
        if (punishment) {
            MinecraftPunishmentModel.update({ _id: req.body.id }, {$set: { reverted: true }}, function(err) {
                if (err) console.log(err);
                var ids = [];
                ...
                async.eachSeries(ids, function (id, next) {
                    MinecraftUserModel.findOne({_id: id}, function (err, user) {
                        loadedUsers.push({
                            name: user.name,
                            id: user._id
                        });
...
router.post('/mc/player/punishments', verifyServer, function(req, res) {
    var result;
    if (req.body.ip) result = MinecraftUserModel.find({ips: req.body.ip});
    else if (req.body.name) result = MinecraftUserModel.find({nameLower: req.body.name.toLowerCase()}).sort('-lastOnlineDate').limit(1);
    else {
        res.json({notFound: true});
        return;
    }
    result.exec(function(err, users) {
        ...
        async.eachSeries(users, function (user, next) {
            MinecraftPunishmentModel.find(req.body.ip ? {punished: user._id, ip: req.body.ip} : {punished: user._id}).exec(function(err, punishments) {
                if (err) console.log(err);
            ...
            async.eachSeries(ids, function (id, next) {
                MinecraftUserModel.findOne({_id: id}, function(err, foundUser) {
                    if (foundUser) {
...
router.get('/mc/punishment/latest', function(req, res) {
    ...
    MinecraftPunishmentModel.find({reverted: false}).sort("-issued").limit(limit).exec(function (err, puns){
        var punishments = new Array();
        async.eachSeries(puns, function (pun, next) {
            var punishment = pun.toJSON();
            MinecraftUserModel.findOne({_id: pun.punisher}, function(err, punisher){
                if (err) console.log(err);
                if (punisher) {
                ...
                MinecraftUserModel.findOne({_id: pun.punished}, function(err, punished){
                    if (err) console.log(err);
                    if (punished) {
