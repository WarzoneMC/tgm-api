import { prop, Typegoose, ModelType, Ref, staticMethod, instanceMethod } from '@hasezoey/typegoose';
import { COLLECTION_PUNISHMENTS } from '../constants';
...
export class Punishment extends Typegoose {
    ...
    static async findByPlayer(this: ModelType<Punishment>, player: Player) {
        return this.findOne({punished: player["_id"]}).populate('punished punisher').exec();
    }

    @staticMethod
    static async findById(this: ModelType<Punishment>, id: string) {
        return this.findOne({_id: id}).populate('punished punisher').exec();
    }
...
export default new Punishment().getModelForClass(Punishment, {
	schemaOptions: {
        collection: COLLECTION_PUNISHMENTS,
        toJSON: {
